...
#define IS_SVR_REV(svr, maj, min) ((SVR_MAJ(svr) == maj) && (SVR_MIN(svr) == min))
...
#define mfspr(rn)                                         \
  ({                                                      \
    unsigned int rval;                                    \
    asm volatile("mfspr %0," stringify(rn) : "=r"(rval)); \
    rval;                                                 \
  })
...
#define CPU_TYPE_ENTRY(n, v, nc) \
  { .name = #n, .soc_ver = SVR_##v, .num_cores = (nc), .mask = (1 << (nc)) - 1 }
 ...
 */
#define INIT_MMAP \
  { &init_mm, 0, 0x1000, NULL, PAGE_SHARED, VM_READ | VM_WRITE | VM_EXEC, 1, NULL, NULL }
